...
public class RestSecurityTests {
    ...
    public void shouldGenerateTheToken() throws Exception {
        ...
        connect(user, mvc).andExpect(status().isOk())
                .andDo(mvcResult -> token = extractToken(mvcResult.getResponse(), jwtConfig));
    ...
    private static ResultActions connect(ZDUser user, MockMvc mvc) throws Exception {
        String url = "/auth";
        ObjectMapper mapper = new ObjectMapper();
        mapper.configure(SerializationFeature.WRAP_ROOT_VALUE, false);
        ObjectWriter ow = mapper.writer().withDefaultPrettyPrinter();
        String requestJson = ow.writeValueAsString(user);
        return mvc.perform(post(url).contentType(APPLICATION_JSON).content(requestJson));
